...
try {
  describe("转换模块", function () {
    ...
    it("数字转大写人民币", function () {
      expect(Convertor.numToAmountInWords(102030.0)).toEqual("壹拾万贰仟零叁拾圆整");
      expect("102030".numToAmountInWords).toEqual("壹拾万贰仟零叁拾圆整");
      expect(Convertor.numToAmountInWords(-102030.0)).toEqual("欠壹拾万贰仟零叁拾圆整");
    ...
    it("敏感信息加符号", function () {
      ...
      expect(
        Convertor.sensitivePlusSymbol("18602736144", [
          [3, 7],
          [8, 11],
        ])
      ).toEqual("186****6***");
