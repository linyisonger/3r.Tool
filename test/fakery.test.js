...
try {
    describe('假数据模块', function () {
        it('身份证号码', function () {
            ...
            expect(Verify.isCitizenIdentificationNumber(Fakery.citizenIdentificationNumber({
                regionCodes: ['410728']
            }))).toEqual(true)
            expect(Verify.isCitizenIdentificationNumber(Fakery.citizenIdentificationNumber({
                regionCodes: ['410728'],
            ...
            expect(Convertor.citizenIdentificationNumberParse(Fakery.citizenIdentificationNumber({
                regionCodes: ['410728'],
                maxAge: 20,
                minAge: 15,
        ...
        it('社会统一信用代码', function () {
            ...
            expect(Verify.isUnifiedSocialCreditIdentifier(Fakery.usci())).toEqual(true)
            expect(Verify.isUnifiedSocialCreditIdentifier(Fakery.usci())).toEqual(true)
            expect(Verify.isUnifiedSocialCreditIdentifier(Fakery.usci({
                regionCodes: ['410728']
            }))).toEqual(true)
            expect(Verify.isUnifiedSocialCreditIdentifier(Fakery.usci({
