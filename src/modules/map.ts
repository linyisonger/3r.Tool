...
export class Map {
    ...
    static distance(now: MapCoordinates, target: MapCoordinates) {
        let x1 = Maths.degreeToRad(now.longitude);
        let y1 = Maths.degreeToRad(now.latitude);
        let x2 = Maths.degreeToRad(target.longitude);
        let y2 = Maths.degreeToRad(target.latitude);
