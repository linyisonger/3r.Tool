...
export class Convertor {
   ...
   * 常见半角字符集
   */
  static HALF_WIDTH_CHAR = `,./<>""''[]...` + "!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~";

  /**
   * 常见全角字符集
   */
  static FULL_WIDTH_CHAR = "，。、《》“”‘’【】…" + "！＂＃＄％＆＇（）＊＋，－．／：；＜＝＞？＠［＼］＾＿｀｛｜｝～";
   ...
   * @param str
   */
  static toHalfWidthChar(str: string) {
    const strArr = str.split("");
    for (let i = 0; i < strArr.length; i++) {
      const c = strArr[i];
      const idx = this.FULL_WIDTH_CHAR.indexOf(c);
      if (idx > -1) strArr[i] = this.HALF_WIDTH_CHAR[idx];
    }
    return strArr.join("");
  }
   ...
   * @param str
   */
  static toFullWidthChar(str: string) {
    const strArr = str.split("");
    for (let i = 0; i < strArr.length; i++) {
      const c = strArr[i];
      const idx = this.HALF_WIDTH_CHAR.indexOf(c);
      if (idx > -1) strArr[i] = this.FULL_WIDTH_CHAR[idx];
    }
    return strArr.join("");
  }
}
