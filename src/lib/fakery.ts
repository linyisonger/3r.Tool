...
function getUSCICheckCode(usciBefore: string) {
	...
	const sum = Maths.sum(usciBefore.split('').map((c, i) => codeCharacterSet.findIndex(v => c === v) * Math.pow(3, i) % 31)) % 31
	const cv = sum === 0 ? 0 : 31 - sum
...
export class Fakery {
	...
	static usci(param: IUSCIParam) {
		...
		for (let i = 0; i < 8; i++) { code += codeCharacterSet[Randoms.int(0, codeCharacterSet.length)] }
	...
	static fullName(param: INameParam) {
		...
		const fullName = familyName[Randoms.int(0, familyName.length)]
		if (isMale && isFemale) {
	...
	static bankCardNumber(param: IBankCardNumberParam) {
		const bin = param?.bin ?? '621210'
		const length = param?.length ?? 18
		let result = bin
