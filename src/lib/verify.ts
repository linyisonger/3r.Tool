...
export class Verify {
  ...
  /** 常见键盘字符 */
  static readonly COMMON_KEYBOARD_CHARACTERS = /[`~!@#$%^&*()_\-+=<>?:"{}|,.\/;'\\[\]·~！@#￥%……&*（）——\-+={}|《》？：“”【】、；‘'，。、]/im;
  /**
   * 像是社会统一信用代码
   * @param usci 社会统一信用代码
   * @returns
   ...
   * @param minLevel 最小难度
   */
  static passwordDifficulty(password: string, minLevel: number | undefined) {
    let level = 0;
    [/[0-9]/, /[a-z]/, /[A-Z]/, Verify.COMMON_KEYBOARD_CHARACTERS].forEach((reg) => reg.test(password) && level++);
    return minLevel ? minLevel <= level : level;
  }
}

const ignorePrototype = ["passwordRules", "versionComparison"];
[
  ...Reflect.ownKeys(Verify)
    .filter((_) => typeof (Verify as any)[_] === "function" && !ignorePrototype.includes(_ as string))
    .map((n) =>
      Object({
        name: n,
